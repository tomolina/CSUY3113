...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("The Coin", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
...
//        }
    ...
    else if ((keys[SDL_SCANCODE_UP]) && (gameOver == 0) && (winner == 0)){
        currentScene->state.player->movement.y = 1.0f;
        if ((currentScene->state.player->position.y < -5) && (!currentScene->state.player->collidedTop)){
            currentScene->state.hearts[0].position.y += .05f;
            currentScene->state.hearts[1].position.y += .05f;
            currentScene->state.hearts[2].position.y += .05f;
        }
        currentScene->state.player->animIndices = currentScene->state.player->animUp;

    }
    
    else if ((keys[SDL_SCANCODE_DOWN]) && (gameOver == 0) && (winner == 0)){
        currentScene->state.player->movement.y = -1.0f;
        if ((currentScene->state.player->position.y < -5) && (!currentScene->state.player->collidedBottom)){
            currentScene->state.hearts[0].position.y -= .05f;
            currentScene->state.hearts[1].position.y -= .05f;
            currentScene->state.hearts[2].position.y -= .05f;
        }
...
//    if((currentScene->state.player->upCoin) || (currentScene->state.player->leftCoin) || (currentScene->state.player->rightCoin) || (currentScene->state.player->bottomCoin)){
//
//        Mix_PlayChannel(-1, bounce, 0);
//        coinCount -= 1;
//
...
void Update() {
    ...
    if (currentScene->state.player->position.y < -5) {
        viewMatrix = glm::translate(viewMatrix, glm::vec3(-5.01, -currentScene->state.player->position.y, 0));
    }
    else {
        viewMatrix = glm::translate(viewMatrix, glm::vec3(-5, 3.5, 0));
       // viewMatrix = glm::rotate(viewMatrix, .45f, glm::vec3(1.0f));
    }
    
//    else if (currentScene->state.player->position.y <= -2){
//        viewMatrix = glm::translate(viewMatrix, glm::vec3(-5, -2, 0));
//       // viewMatrix = glm::rotate(viewMatrix, .45f, glm::vec3(1.0f));
//    }
