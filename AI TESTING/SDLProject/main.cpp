...
void Initialize() {
    ...
    for(int i = 0; i < WINNER_COUNT; i++){
        state.correctPlat[i].Update(0, NULL, NULL, 0);
    }
    ...
    state.enemies[0].position = glm::vec3(4, -2.75, 0);
...
void Update() {
    ...
    while (deltaTime >= FIXED_TIMESTEP) {
        ...
        state.player->Update(FIXED_TIMESTEP, state.player, state.correctPlat, WINNER_COUNT);
